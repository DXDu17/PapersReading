...
CityFlow数据集：由333辆汽车的36935张训练图像组成。私有测试集包含1052个查询（query ）图像和18290个库（gallery ）图像。组织者还提供了来自文35的合成数据，包括1362辆汽车的192150张图像。由于没有提供验证集，因此从训练集中拆分了一个验证集，以做重要部件的消融研究。遵循文38的拆分，将 最后78个车辆ID作为验证集。
...
所提方法达到了SOTA mAP精度，即84.13%，大大优于第二名团队，并验证了所提reid方法的有效性。

<div align=center><img src="../images/Going_Beyond_Real_Data_A_Robust_Visual_Representation/table1.png" width="416" height="194"/></div>

##### 合成数据的影响

在验证集上评估合成数据的有效性。分别对使用合成数据和不使用合成数据的模型进行了训练和评估。用合成数据训练的模型Rank@1达到了80.86%，mAP为46.90%，优于不使用合成数据训练的模型。

<div align=center><img src="../images/Going_Beyond_Real_Data_A_Robust_Visual_Representation/table2.png" width="408" height="121"/></div>

##### 后处理效果

在验证集上评估了所提后处理方法。逐渐添加了后处理技术，产生了优越的性能。在验证集上，重排序后将基线的mAP从46.90%提高到61.26%。

<div align=center><img src="../images/Going_Beyond_Real_Data_A_Robust_Visual_Representation/table3.png" width="408" height="181"/></div>
...
分别在没有或有后处理的情况下可视化排名列表。在测试集中选择了一些难负样本。从排名列表的变化中观察到，尽管模型已经非常强大，但仍然存在大量的查询，由于有相似姿态的难负样本，这些查询无法找到正确的匹配项，借助后处理可以得到纠正。

##### 可视化特征图

进一步可视化学习模型的热度图。给定输入图像，按照文40的可视化方法计算最终池化层之前的激活总和。学习的模型更多地关注有区别的部分，如车灯，这与人类经验是一致的。因此，车辆表示对由较大视点变化而引起的视觉外观变化具有鲁棒性。

<a name="6"></a>

## 6. 总结

开发了一个车辆reid系统，在AICity Challenge 2020的reid赛道中获得第一名。验证了合成数据在鲁棒性视觉表示学习中的有效性，并在车辆表示学习的背景下探索了不同的基线和生成模型。未来将继续研究3D车辆模型和其他相关技术，以促进车辆reid在现实世界中的应用。
